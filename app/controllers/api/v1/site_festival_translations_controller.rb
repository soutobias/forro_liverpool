...
module Api
  module V1
    class SiteFestivalTranslationsController < ApplicationController
      before_action :set_site, only: %i[show update destroy]

      # GET /sites
      def index
        @site_festival_translations = SiteFestivalTranslation.all
      ...
      # POST /sites
      def create
        @site_festival_translation = LiverpoolQuestionTranslation.new(question_params)
      ...
      def set_site
        @site_festival_translation = LiverpoolQuestionTranslation.find(params[:id])
      end
